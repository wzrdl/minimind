"""
MiniMind 全量监督微调（Full SFT）脚本

该脚本用于对预训练模型进行全量监督微调（Supervised Fine-Tuning）。
与预训练不同，SFT使用指令-回答格式的数据，学习遵循指令和生成高质量回答。

主要特点：
1. 使用更小的学习率（通常5e-7到1e-6），避免破坏预训练知识
2. 使用SFTDataset，处理指令-回答格式的数据
3. 支持分布式训练、混合精度、梯度累积
4. 自动检查点保存和恢复
...
与预训练的区别：
- 学习率更小：SFT需要精细调整，避免过拟合
- 数据格式：使用指令-回答对，而非纯文本
- 训练轮数：通常2-5轮即可，不需要太多轮次
"""
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    """
    训练一个epoch的核心函数（SFT版本）
    
    该函数执行一个完整的训练epoch，包括：
    1. 前向传播和损失计算（只计算回答部分的损失）
    2. 反向传播和梯度累积
    3. 梯度裁剪和参数更新
    4. 日志记录和检查点保存
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            # 应用损失掩码，只计算回答部分的损失
            # loss * loss_mask: 将指令部分的损失置为0（loss_mask为0的位置）
            # .sum() / loss_mask.sum(): 计算回答部分的平均损失
            # 这样模型只学习如何生成回答，而不学习如何重复指令
...
if __name__ == "__main__":
    ...
    # 训练超参数
    # SFT通常只需要2-5轮训练即可，不需要太多轮次
    parser.add_argument("--epochs", type=int, default=2, help="训练轮数（SFT通常2-5轮即可）")
    
    # SFT通常使用较小的batch size，因为数据质量更重要
    parser.add_argument("--batch_size", type=int, default=16, help="batch size（SFT通常使用较小batch size）")
    
    # SFT使用很小的学习率，避免破坏预训练知识
    # 通常范围：5e-7 到 1e-6，比预训练小100-1000倍
    parser.add_argument("--learning_rate", type=float, default=5e-7, help="初始学习率（SFT需要很小，通常5e-7到1e-6）")
    ...
    # 数据相关参数
    # SFT数据格式：JSONL文件，每行包含instruction和output字段
    parser.add_argument("--data_path", type=str, default="../dataset/sft_mini_512.jsonl", help="训练数据路径（JSONL格式，包含instruction和output）")
    
    # 权重加载参数
    # SFT通常基于预训练权重开始训练
    parser.add_argument('--from_weight', default='pretrain', type=str, help="基于哪个权重训练，为none则不基于任何权重训练（SFT通常基于pretrain）")
    ...
    # SFT通常基于预训练权重（from_weight='pretrain'）开始训练
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)
    
    # 创建SFT数据集
    # SFTDataset处理指令-回答格式的数据，生成loss_mask（只对回答部分为1）
    train_ds = SFTDataset(
        args.data_path, 
        tokenizer, 
